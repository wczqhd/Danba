...
msgStorage.saveReceiveMsg = function(receiveMsg, type){
   ...
   // console.log('msgstorage  receive ', receiveMsg);
		sendableMsg = {
			id: receiveMsg.id,
			type: type,
...
msgStorage.saveMsg = function(sendableMsg, type, receiveMsg){
 //   console.log('sendableMsgsendableMsg', receiveMsg)
	let me = this;
	...
	let renderableMsg = msgPackager(sendableMsg, type, myName);//这里赋值了mid
	...
	save();
    function save() {
   //     console.log("storage  %%%%  ", curChatMsg);
     //   console.log("storage  $$$$   ", sessionKey);
		wx.setStorage({
			key: sessionKey,
			data: curChatMsg,
			success(){
