...
Component({
	...
	methods: {
		audioPlay(){
			...
			var curl = ''
             ...
             curl = this.data.msg.body.body.url;
          console.log('音频本地', curl);
	...
	attached(){
		this.setData({
			time: this.properties.msg.body.length + "''",
          style: "self"
		})
	},
	moved(){},
	...
	ready(){
		let self = this
       let curl = ''
   //    console.log("audiosimple @@@@@", this.data.msg);
		let audioCtx = this.data.__comps__.audioCtx = audioCtxFc.getCtx(this.data.msg.mid);
  //     console.log("audiosimple @@@@@", this.data.msg.mid);
