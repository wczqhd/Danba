	...
	methods: {
		...
		toggleRecordModal(gid){
          ...
          let uname = {};
			...
			{
				 if( getApp().globalData.groupList[i].groupid == gid)
             {
                 uname.groupId = gid;
                 uname.myName = wx.getStorageSync("DanbaId");//因为这个是要被用做唯一标识的，不能用wx.getStorageSync("myUsername")
								 uname.your = getApp().globalData.groupList[i].groupname;
								 console.log('Describe audio index &&  ',getApp().globalData.groupList[i].describe);
					this.setData({
		...
		isGroupChat(){
			return true;//this.data.chatType == msgType.chatType.CHAT_ROOM;
		},
	 ...
	 //  console.log("audio mid ",mid);
		...
		uploadRecord(tempFilePath, dur){
           ...
           //     console.log("msgType.chatType.CHAT_ROOM  &&&  ",me.getSendToParam());
